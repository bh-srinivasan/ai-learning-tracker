...
class DeploymentManager:
    ...
    def scan_hardcoded_password_resets(self) -> List[Dict[str, any]]:
        ...
        for file_path in python_files:
            if "deploy_azure_secure.py" in str(file_path):
                continue  # Skip the deployment script itself
        ...
        if issues:
            logger.warning(f"[WARNING] Found {len(issues)} potential security issues:")
            for issue in issues:
                logger.warning(f"    {issue['file']}:{issue['line']} - {issue['pattern']}")
                logger.warning(f"        Match: {issue['match']}")
        else:
            logger.info("[SUCCESS] No hardcoded password reset issues found")
            
        return issues

    def check_protected_user_logic(self) -> bool:
        """Verify that protected user logic is in place"""
        logger.info("[SECURITY] Checking protected user logic...")
        ...
        try:
            ...
            if found_protections >= 2:
                logger.info("[SUCCESS] Protected user logic appears to be properly implemented")
                return True
            else:
                logger.warning("[WARNING] Protected user logic may be incomplete")
                return False
    ...
    def deploy_to_azure(self) -> bool:
        ...
        logger.info("[DEPLOY] Starting Azure deployment...")
        ...
        logger.info("[SUCCESS] Successfully pushed to Azure")
        
        # Note: Environment variables should be set through Azure Portal
        logger.info("[INFO] Environment variables setup:")
    ...
    def run_deployment(self, auto_commit: bool = False, commit_message: str = None) -> bool:
        ...
        logger.info("[DEPLOY] Starting AI Learning Tracker deployment process...")
        logger.info(f"[INFO] Working directory: {self.repo_path}")
        ...
        try:
            ...
            if git_status["has_changes"]:
                if auto_commit:
                    ...
                    if not self.commit_changes(commit_message):
                        logger.error("[ERROR] Failed to commit changes. Aborting deployment.")
                        return False
                else:
                    logger.error("[ERROR] Uncommitted changes detected. Please commit or stash them first.")
                    logger.info("[INFO] Use --auto-commit flag to automatically commit changes")
                    return False
            ...
            if security_issues:
                ...
                if high_severity:
                    logger.error("[ERROR] High severity security issues found. Please fix them before deployment.")
                    return False
                else:
                    logger.warning("[WARNING] Medium severity issues found but continuing deployment...")
            
            # Step 3: Check protected user logic
            if not self.check_protected_user_logic():
                logger.warning("[WARNING] Protected user logic verification failed, but continuing...")
            
            # Step 4: Deploy to Azure
            if not self.deploy_to_azure():
                logger.error("[ERROR] Azure deployment failed")
                return False
            
            logger.info("[SUCCESS] Deployment completed successfully!")
            logger.info(f"[INFO] Application URL: https://{self.azure_app_name}.azurewebsites.net")
            logger.info(f"[INFO] Admin Panel: https://{self.azure_app_name}.azurewebsites.net/admin")
            
            return True
            
        except Exception as e:
            logger.error(f"[ERROR] Deployment failed with error: {e}")
            return False
...
def main():
    ...
    if args.security_only:
        logger.info("[SECURITY] Running security scan only...")
        ...
        if not issues and protected_ok:
            logger.info("[SUCCESS] Security scan completed - no issues found")
            return 0
        else:
            logger.warning("[WARNING] Security scan completed - issues found") 
            return 1
    else:
